...
This file is part of Sprout.
   ...
   Modification: 2014/06/19 - fix show flag for backend rather than os
...
class parameter_view(Frame): 
    def __init__(self,master=None): 
       ...
       ##SECOND BAR - GLOBAL OPTIONS LINE 1 (PSF)
        ...
        self.minflux=StringVar()
        self.maxflux=StringVar()
        ...
        mb=Label(fm,text="Min: ").pack(side=LEFT,anchor=W)
        mb=Entry(fm,textvariable=self.minflux,width=10).pack(side=LEFT,anchor=W)
        mb=Label(fm,text="max: ").pack(side=LEFT,anchor=W)
        mb=Entry(fm,textvariable=self.maxflux,width=10).pack(side=LEFT,anchor=W)
        fm.pack()
        bound.pack(fill=X)
...
##------------------------------------------------------------------------
    ...
    def plot_file(self,h,rho,beta,angle_ind,batch,ax):
        ...
        if(matplotlib.get_backend() != 'macosx'):  ##MAC SPECIFIC
          ion()
        ...
        if((self.minflux.get()=="") | (self.maxflux.get()=="")):
           ...
           self.minn=float(self.minflux.get())
           self.maxx=float(self.maxflux.get())
